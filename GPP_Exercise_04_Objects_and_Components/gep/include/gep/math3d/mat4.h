...
namespace gep
{
    ...
    struct mat4_t
    {
        ...
        static const mat4_t<T> ortho(T pLeft, T pRight, T pBottom, T pTop, T pNear, T pFar)
        {
            ...
            res.data[8] = 0;
            res.data[9] = 0;
            res.data[10] = -2 / (pFar - pNear);
            res.data[11] = 0;

            res.data[12] = -(pRight + pLeft) / (pRight - pLeft);
            res.data[13] = -(pTop + pBottom) / (pTop - pBottom);
            res.data[14] = -(pFar + pNear) / (pFar - pNear);
            res.data[15] = 1;
            return res;
        }
        ...
        static const mat4_t<T> lookDirMatrix(const vec3_t<T>& dir, const vec3_t<T>& pUp)
        {
            ...
            mat4_t<T> res(DO_NOT_INITIALIZE);
            z = dir.normalized();
            x = z.cross(pUp.normalized()).normalize();
            y = x.cross(z);
            z = z * -1;
