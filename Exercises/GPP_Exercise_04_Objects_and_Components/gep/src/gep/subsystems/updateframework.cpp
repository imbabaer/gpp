...
gep::UpdateFramework::UpdateFramework() :
    ...
    , m_gameThread(this)
{
    memset(m_pFrameTimesArray, 0, sizeof(m_pFrameTimesArray));
}
...
float gep::UpdateFramework::calcElapsedTimeAverage(size_t numFrames) const
{
    ...
    size_t numValues = 0;
    size_t tempFrameIdx = m_frameIdx;

    while (numValues < numFrames)
    {
        float frameTime = m_FrameTimesPtr[tempFrameIdx];
        if (frameTime == 0.0f) break;
    ...
    GEP_ASSERT(numValues > 0);
    return averageTime / float(numValues);
}
