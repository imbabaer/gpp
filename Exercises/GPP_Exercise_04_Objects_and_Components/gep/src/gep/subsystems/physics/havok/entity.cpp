...
gep::HavokRigidBody::HavokRigidBody(hkpRigidBody* rigidBody /*= nullptr*/) :
    HavokEntity(rigidBody),
    m_pTriggerVolume(nullptr)
{
    GEP_ASSERT(rigidBody && m_pEntity, "Must not pass a nullptr!");
}
...
gep::HavokRigidBody::HavokRigidBody(const RigidBodyCInfo& cinfo) :
    ...
    m_shape(cinfo.ownsShape ? cinfo.shape : nullptr)
{
    ...
    conversion::hk::toHkVector4(cinfo.linearVelocity, hkcinfo.m_linearVelocity);
    conversion::hk::toHkVector4(cinfo.angularVelocity, hkcinfo.m_angularVelocity);
    ...
    m_pEntity = new hkpRigidBody(hkcinfo);
...
{
    m_pEntity->setUserData(reinterpret_cast<hkUlong>(this));
}

void gep::HavokRigidBody::destroy()
{
    m_pEntity->setUserData(0);
    m_pTriggerVolume = nullptr;
}
