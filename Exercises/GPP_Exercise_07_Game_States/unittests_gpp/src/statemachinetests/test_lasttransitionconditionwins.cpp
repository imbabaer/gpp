...
GEP_UNITTEST_TEST(StateMachine, LastTransitionConditionWins)
{
    ...
    // is actually run.
    {
        ...
        pState_A->setLeaveCondition([](){ return true; });
        pState_B->setLeaveCondition([](){ return true; });
        pState_C->setLeaveCondition([](){ return true; });
        pState_D->setLeaveCondition([](){ return true; });
        ...
        pState_A->getLeaveEvent()->registerListenerPriority(-100, [&](State::LeaveEventData* pData){
            pData->setNextState(pState_B);
            return gep::EventResult::Handled;
        });
        // Will be called second
        pState_A->getLeaveEvent()->registerListenerPriority(0, [&](State::LeaveEventData* pData){
        ...
        pState_B->getEnterEvent()->registerListener([](State::EnterEventData*){
            GEP_ASSERT(false, "Unexpected state change!");
            return gep::EventResult::Handled;
        });
        pState_C->getEnterEvent()->registerListener([](State::EnterEventData*){
            GEP_ASSERT(false, "Unexpected state change!");
            return gep::EventResult::Handled;
        });
    }
